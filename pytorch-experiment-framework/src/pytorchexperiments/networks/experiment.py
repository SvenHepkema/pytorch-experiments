...
from pytorchexperiments.pytorchutils.argparseutils import get_training_parameters_from_args
from pytorchexperiments.pytorchutils.training import ( evaluate_network, train_network)
from pytorchexperiments.pytorchutils.dataclasses import TrainingParameters, TrainingPerformance, ValidationPerformance
...
class Experiment:
    ...
    def _generate_dataloader(self) -> DataLoader:
        ...
        return DataLoader(list(zip(data_tensor, labels_tensor)), shuffle=True, batch_size=self.args.batch_size)

    def run(self) -> tuple[TrainingParameters, TrainingPerformance, ValidationPerformance]:
        training_params = get_training_parameters_from_args(self.args)
        training_dataloader = self._generate_dataloader()
        training_perf, network = train_network(training_dataloader, training_params, self.network_generator)

        validation_data_tensor = self.data_to_tensor_converter(self.data_generator(self.args.validation_size))
        validation_perf = evaluate_network(validation_data_tensor, network, self.function_evaluator)
